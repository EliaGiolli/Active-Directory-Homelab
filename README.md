...
- [x] Design and create an OU structure (not a flat dump of users)
- [x] Create users and groups organized by the OU structure
...
- Created two security groups in the wrong OU by right-clicking the wrong tree node — recovered with `Move...` instead of delete-and-recreate, verified independently with `Get-ADGroup`.
- "Installed" VMware Tools that turned out not to be running at all (`Get-Service VMTools` couldn't even find the service) — the host-side "Install VMware Tools" menu option only mounts the installer; running it inside the guest is a separate, manual step.
...
- [ ] **PowerShell automation pass** — revisit key steps (OU creation, GPO backup/import) as scripts in `scripts/`, now that bulk user creation is already scripted
